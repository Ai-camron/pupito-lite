...
export default function OnlineStoreCallout({ variant = 'default', className = '' }: OnlineStoreCalloutProps) {
  if (variant === 'compact') {
    return (
      <motion.div
        ...
        className={`bg-linear-to-r from-[#1E90FF]/10 via-[#22d3ee]/10 to-[#FFD700]/10 border border-[#22d3ee]/20 rounded-xl p-4 ${className}`}
      >
        <div className="flex items-center justify-center gap-2 text-center">
          <Globe className="w-4 h-4 text-[#00FFFF] shrink-0" />
          <p className="text-sm text-gray-300">
            <span className="font-semibold text-[#22d3ee]">Online Only Store</span> • Made-to-order via {STORE_INFO.fulfillmentPartner} • Ships worldwide
          </p>
        </div>
      </motion.div>
    )
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className={`bg-linear-to-br from-[#1A1A1A]/80 to-[#2A2A2A]/60 border border-[#22d3ee]/30 rounded-2xl p-6 backdrop-blur ${className}`}
    >
      ...
      <div className="space-y-4">
        <p className="text-gray-300 leading-relaxed">
          PUPITO is a <span className="text-[#22d3ee] font-semibold">100% online store</span> - we don&apos;t have physical retail locations. 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
          <div className="flex items-start gap-3">
            <Package className="w-5 h-5 text-[#22d3ee] shrink-0 mt-0.5" />
            <div>
